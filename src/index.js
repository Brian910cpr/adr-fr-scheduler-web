BEGIN FILE: src/index.js
// ShiftCommander — merged API + Diagnostics + Bulk Import (Cloudflare Worker, JS)
// D1 binding: env.DB (adr_fr)
// Routes:
// GET /api/users
// POST /api/users {name,role,can_drive?,notes?,member_no?}
...
// POST /api/seed (migrate + demo users + 60d shifts)
// WS /ws (broadcast SYNC)
// GET /test/schema/users
// GET /test/users
// POST /api/users/bulk?truncate=1&token=YOUR_TOKEN (text/plain CSV)
...
try {
  ...
  if (url.pathname === "/ws") {
    if (request.headers.get("Upgrade") !== "websocket") {
      return json({ error: "upgrade required" }, 426, origin);
    }
  ...
  if (url.pathname === "/api/seed" && request.method === "POST") {
    ...
    return json({ ok: true, now: new Date().toISOString(), seen_origin: origin }, 200, origin);
  }

  // Users
  if (url.pathname === "/api/users" && request.method === "GET") {
    await migrate(env.DB); // idempotent
  ...
  if (url.pathname === "/api/users" && request.method === "POST") {
    ...
    await env.DB.prepare("INSERT OR IGNORE INTO prefs(user_id,prefer24s,notes) VALUES(?,?,?)")
      .bind(id, 0, "").run();
  ...
  if (url.pathname === "/api/availability" && request.method === "POST") {
    ...
    if (!body.userId || !body.date || !body.half || !body.state) {
      return json({ error: "userId,date,half,state required" }, 400, origin);
    }
    await env.DB.prepare(
  ...
  if (url.pathname === "/api/shift/assign" && request.method === "POST") {
    ...
    const row = await env.DB.prepare("SELECT assignees, status FROM shifts WHERE date=? AND half=?")
      .bind(body.date, body.half).first();
    ...
    await env.DB.prepare("UPDATE shifts SET assignees=?, status=? WHERE date=? AND half=?")
      .bind(JSON.stringify(assignees), nextStatus, body.date, body.half).run();
  ...
  if (url.pathname === "/api/shift/status" && request.method === "POST") {
    ...
    await ensureShift(env.DB, body.date, body.half);
    await env.DB.prepare("UPDATE shifts SET status=? WHERE date=? AND half=?")
      .bind(body.status, body.date, body.half).run();
    hub.broadcast({ type: "SYNC", changed: "shift", date: body.date, half: body.half });
    return json({ ok: true }, 200, origin);
  }
  ...
  if (url.pathname === "/test/users" && request.method === "GET") {
    ...
    const res = await env.DB.prepare(
      "SELECT id,name,role,member_no,can_drive,notes FROM users ORDER BY name LIMIT 25"
    ).all();
...
if (!row) await DB.prepare("INSERT INTO shifts(date,half,assignees,status) VALUES(?,?,?,?)")
...
await DB.prepare(CREATE TABLE IF NOT EXISTS users ( id TEXT PRIMARY KEY, name TEXT NOT NULL, role TEXT NOT NULL, member_no TEXT DEFAULT '', can_drive TEXT DEFAULT '[]', notes TEXT DEFAULT '' )).run();

await DB.prepare(CREATE TABLE IF NOT EXISTS prefs ( user_id TEXT PRIMARY KEY, prefer24s INTEGER DEFAULT 0, notes TEXT DEFAULT '' )).run();

await DB.prepare(CREATE TABLE IF NOT EXISTS availability ( user_id TEXT NOT NULL, date TEXT NOT NULL, half TEXT CHECK(half IN ('AM','PM')) NOT NULL, state TEXT CHECK(state IN ('unset','prefer','available','no')) NOT NULL, PRIMARY KEY (user_id, date, half) )).run();

await DB.prepare(CREATE TABLE IF NOT EXISTS shifts ( date TEXT NOT NULL, half TEXT CHECK(half IN ('AM','PM')) NOT NULL, assignees TEXT NOT NULL DEFAULT '[]', status TEXT CHECK(status IN ('unassigned','proposed','approved')) NOT NULL DEFAULT 'unassigned', PRIMARY KEY (date, half) )).run();
}
...
await DB.prepare("INSERT OR IGNORE INTO prefs(user_id,prefer24s,notes) VALUES(?,?,?)")
.bind(u.id, 0, "").run();
}
const now = new Date();
for (let i=0;i<60;i++){
...
const hub = {
sockets: new Set(),
accept(ws){ ws.accept(); this.sockets.add(ws);
